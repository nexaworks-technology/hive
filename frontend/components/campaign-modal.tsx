...
const API_BASE_URL = process.env.NEXT_PUBLIC_BACKEND_URL || 'https://hive-cot2.onrender.com';
...
/**
 * Generate subject line using proven high-open-rate formulas
 * Formula 1: Just first name (simplicity & personalization)
 * Formula 2: "Thoughts [FirstName]?" (opinion-seeking, curious)
 * Formula 3: "[FirstName] X [SenderName]" (collaboration/referral hint)
 */
function generateSubjectLine(prospect: any, userProfile: any, emailType: string = 'initial'): string {
  const firstName = getFirstName(prospect.name || 'there');
  const senderFirstName = getFirstName(userProfile?.name || 'SutraHR');
  
  if (emailType === 'initial') {
    return firstName; // Formula 1: Simple, personal, high engagement
  } else if (emailType === 'followup-day-2') {
    return `Thoughts ${firstName}?`; // Formula 2: Opinion-seeking
  } else if (emailType === 'followup-day-4') {
    return `${firstName} X ${senderFirstName}`; // Formula 3: Collaboration hint
  } else {
    return firstName; // Formula 1: Final follow-up - back to simplicity
  }
}
...
function generateFollowUpEmail(prospect: any, userProfile: UserProfile | Record<string, any>, dayNumber: number): { html: string; subject: string; body: string } {
  ...
  const subjectLine = dayNumber === 2 ? generateSubjectLine(prospect, profile, 'followup-day-2') : dayNumber === 4 ? generateSubjectLine(prospect, profile, 'followup-day-4') : generateSubjectLine(prospect, profile, 'followup-day-6');
...
export default function CampaignModal({ isOpen, onClose }: CampaignModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        const gmailCheckRes = await fetch(`${API_BASE_URL}/check-google-tokens`, {
          headers: {
      ...
      const res = await fetch(`${API_BASE_URL}/campaigns-v2/create`, {
        method: 'POST',
        headers: { 
      ...
      const pollInterval = setInterval(async () => {
        pollCount++;
        try {
          const campaignRes = await fetch(`${API_BASE_URL}/campaigns-v2/${campaignId}`);
          ...
          if (scraped.length > 0) {
            ...
            for (const lead of scraped) {
              try {
                ...
                const subject = generateSubjectLine(lead, profile, 'initial');
                
                console.log(`[campaign-modal] 📤 Sending initial email to ${lead.email}...`);
                console.log(`[campaign-modal] 📧 Subject: "${subject}"`);
                const sendRes = await fetch(
                  `${API_BASE_URL}/campaigns-v2/${campaignId}/prospects/${lead.id}/send-email`,
                  {
                    method: 'POST',
                    headers: { 
                      'Content-Type': 'application/json',
                ...
                if (sendRes.ok) {
                  ...
                  for (const day of followupDays) {
                    try {
                      ...
                      await fetch(
                        `${API_BASE_URL}/campaigns-v2/${campaignId}/prospects/${lead.id}/schedule-followup`,
                        {
                          method: 'POST',
                          headers: {
                            'Content-Type': 'application/json',
